...
class XML_parsing():
    ...
    def XML_ActiveParsing(self):
        ...
        # HDDL Requirement list for the specific domain file
        ...
        # Edges List
        edge_list = []
        # Feedback edge list - it saves the edges that are ill defined to a different list that the normal edges
        edge_list_feedback = []
        ...
        # How to use BeautifulSoup:
            # Finding all instances of tag - You get a list with all the instances with that tag:
                # b_node = SysML_data.find_all('node')
        ...
        b_packagedElement = SysML_data.find_all('packagedElement') 
        b_nestedClassifier = SysML_data.find_all('nestedClassifier') 
                ...
                for child in element.children:
                    ...
                    try:
                        if not isinstance(child, str) and child.attrs['name'] == 'ParamSpec':
                            ...
                            element['name'] = '{} - {}'.format(element['name'], element_type)
        ...
        b_comments = SysML_data.find_all('ownedComment') 
        self.xml_parsing_output["b_comments"] = b_comments
        # Find all the nodes
        b_nodes = SysML_data.find_all('node')
