...
class MBSE_HDDL_TRANSLATION_VITECH():
    def __init__(self, domain_name, Number, Class, Name, Inputs, Outputs, Decomposed_by, Decomposes, Category, Description, Iterations):
    ...
    def domain_file_VITECH(self):
        ...
        temp_string_added = []
        # List of predicate to add to the HDDL doain file 
        predicate_list_hddl = []
            ...
            if ii == 'Function':   
                ...
                if temp_string[0] == 'm':
                    ...
                    try:
                        ...
                        try:
                            iterations = self.Iterations[counter_for_loop][-1]
                            for pp in range(0,int(iterations)):
                                for nn in decomposing_actions.split('\n'):
                                    nn = nn.strip()
                                    method_decomposed_list.append('\n \t\t (task{} ({} [] ))  '.format(counter_ordering_methods,nn))
                        ...
                        method_list_hddl.append( \
                                                '(:method {} \n\t :parameters({}) \n\t :task({} []) \n\t :precondition(and \n {}) \n\t \n\t :subtasks(and {}) \n\t :ordering(and {} \n)'.format(method,"".join(method_parameters), decomposed_task, "".join(method_inputs_list), "".join(method_decomposed_list), "".join(method_ordering_list)))

                        
                    except:
                        print('no decomposition')
                        method_list_hddl.append( \
                                                '(:method {} \n\t :parameters({}) \n\t :task({} []) \n\t :precondition(and \n {}) \n\t :subtasks( ) \n\t :ordering( ) \n)'.format(method,"".join(method_parameters), decomposed_task, "".join(method_inputs_list)))
...
def main():
    ...
    Iterations = mbse_db['relates to Targets'].tolist()
    ...
    domain_name = 'rover'
    file = MBSE_HDDL_TRANSLATION_VITECH(domain_name, Number, Class, Name, Inputs, Outputs, Decomposed_by, Decomposes, Category, Description, Iterations)
