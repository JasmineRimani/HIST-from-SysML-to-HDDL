...
class FeedbackDefinition():
    ...
    def FeedbackFile(self): 
        ...
        # with open(self.d_now + '//outputs//' + self.domain_name, 'r') as domain_file_from_xml:
        #     domain_file_from_xml_line = list(line for line in (l.replace(" ", "").replace("\t", "").replace("\n", "")  for l in domain_file_from_xml) if line)

        # with open(self.d_now + '//inputs//' + self.feedback_file_name, 'r') as feedback_file:
        #     feedback_file_line = list(line for line in (l.replace(" ", "").replace("\t", "").replace("\n", "") for l in feedback_file) if line)
        
        
        # # Use as a Counter
        # dummy_list = []
        
        # for ii in range(len(feedback_file_line)):
        #     if feedback_file_line[ii] not in domain_file_from_xml_line:
        #         dummy_list.append(feedback_file_line[ii])
